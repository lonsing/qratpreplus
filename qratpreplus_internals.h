...
#ifndef QRATPREPLUS_INTERNALS_H_INCLUDED
#define QRATPREPLUS_INTERNALS_H_INCLUDED
...
/* Declare lists and stacks of QBlocks, Clauses, etc. */
DECLARE_DLINK (QBlock, QBlock);
DECLARE_DLIST (QBlock, QBlock);
...
/* PCNF object, defined by list of qblocks (quantifier prefix), array of
...
/* QBlock object (quantifier block in the quantifier prefix). */
struct QBlock
{
  QuantifierType type;
  /* QBlocks have nesting level, starting at 0, increases by one from left to
     right. */
  unsigned int nesting;
  /* IDs of variables in a qblock are kept on a stack. */
  VarIDStack vars;
  /* QBlocks appear in a doubly linked list. */
  QBlockLink link;
};
...
/* Check if qblock is existential or universal. */
#define QBLOCK_EXISTS(s) ((s)->type == QTYPE_EXISTS)
#define QBLOCK_FORALL(s) ((s)->type == QTYPE_FORALL)
...
/* QRATPrePlus object. This is used by the main application. */
struct QRATPrePlus
{
  ...
  /* Pointer to most recently opened qblock during parsing. */
  QBlock *opened_qblock;
  /* Flag to indicate whether all qblocks have been parsed. Used to
     start merging of adjacent qblocks of the same quantifier type. */
  ...
  /* Flag to indicate if library is in reset state. If so, then
     another call of 'preprocess' may happen. */
  unsigned int no_reset_state:1;
  /* Flag to indicate that preprocessing was called already. No
     further modifications of formula are allowed, e.g., by adding
     clauses. */
  unsigned int preprocessing_called:1;

  /* Flag to indicate that time limit was exceeded. */
  int time_exceeded;
  
  /* Stack of assignments enqueued for propagation. */
  VarIDStack qbcp_queue;
  
  /* Iterators for exporting qblocks and clauses.  */
  struct
  {
    Clause *cl_iter_p;
    QBlock *qbl_iter_p;
  ...
  /* Formula statistics, before and after preprocessing. */
  struct
  {
    unsigned int before_num_qblocks;
    unsigned int after_num_qblocks;
    unsigned int before_num_clauses;
    unsigned int after_num_clauses;
    unsigned int before_num_univ_lits;
    unsigned int after_num_univ_lits;
    unsigned int before_num_exist_lits;
    unsigned int after_num_exist_lits;
  } formula_stats;
  ...
  struct
  {
    ...
    /* Exploit qblock ordering of literals in clauses to avoid visiting
    ...
    /* For QRAT: when assigning variables from currently tested clause, do not
       ...
       TODO / CHECK: for QRATU the pivot is universal, and the pivot qblock itself may
    ...
    /* Compute formula statistics. */
    unsigned int formula_stats:1;
  } options;
};
